...
void Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, CombatDamage* data)
{
	...
	// Add critical hit, evade, and reflect mechanics here
	if (target->getPlayer() && damage.primary.value < 0) {
		Player* playerTarget = target->getPlayer();

		// Evade check
		if (uniform_random(1, 100) <= playerTarget->getEvadeChance()) {
			damage.primary.value = 0;
			damage.secondary.value = 0;

			// Only send message to the player who evaded
			playerTarget->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Evaded attack!");

			return;
		}

		// Reflect check
		if (uniform_random(1, 100) <= playerTarget->getReflectChance()) {
			if (caster) {
				int32_t reflectDamage = damage.primary.value; // Reflect full damage

				// Simple message for the player who reflected
				std::string attackerName = "the attack";
				if (caster->getPlayer()) {
					attackerName = caster->getName() + "'s attack";
				}

				std::stringstream ss;
				ss << "You reflected " << attackerName << "!";
				playerTarget->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, ss.str());

				// Apply the reflected damage to the attacker
				// The caster becomes the target of the reflected damage
				CombatDamage reflectDmg;
				reflectDmg.primary.value = reflectDamage;
				reflectDmg.primary.type = damage.primary.type;
				reflectDmg.origin = ORIGIN_SPELL;
				g_game.combatChangeHealth(target, caster, reflectDmg);

				// Prevent the player from taking damage when they reflect
				damage.primary.value = 0;
				damage.secondary.value = 0;
	...
	// Critical hit check
	if (caster && caster->getPlayer() && damage.primary.value < 0) {
		Player* playerCaster = caster->getPlayer();

		if (uniform_random(1, 100) <= playerCaster->getCriticalHitChance()) {
			damage.primary.value *= 2; // Double damage for critical hit
			damage.secondary.value *= 2; // Also double secondary damage if any

			// Only send message to the attacker
			playerCaster->sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Critical hit!");
		}
	}
